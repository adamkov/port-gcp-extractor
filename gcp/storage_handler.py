...
class StorageHandler(BaseHandler):
    ...
    def handle(self):
        logger.info(f"List Storage buckkets")
        try:
    ...
    def _handle_list_response(self, buckets_reponse):
        for bucket in buckets_reponse:
            self.handle_single_resource_item(bucket=bucket)
        #with ThreadPoolExecutor(max_workers=consts.MAX_UPSERT_WORKERS) as executor:
        #    futures = [executor.submit(self.handle_single_resource_item, bucket.name, 'upsert') for bucket in buckets_reponse]
#
